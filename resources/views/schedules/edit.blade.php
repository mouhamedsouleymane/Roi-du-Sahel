            ...
            <div class="bg-white shadow-xl rounded-3xl p-8 border border-slate-200/80">
                <form method="POST" action="{{ route('schedules.update', $schedule) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="class_id" required class="w-full text-sm font-bold border-slate-300 rounded-xl shadow-xs focus:ring-purple-500 focus:border-purple-500">
                            @foreach ($classes as $cls)
                                <option value="{{ $cls->id }}" {{ old('class_id', $schedule->class_id) == $cls->id ? 'selected' : '' }}>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-2">Salle</label>
                            <input type="text" name="room_number" value="{{ old('room_number', $schedule->room_number) }}"
                        ...
                        <div>
                            ...
                            <input type="time" name="start_time" required value="{{ old('start_time', substr($schedule->start_time, 0, 5)) }}"
                                   class="w-full text-sm font-bold border-slate-300 rounded-xl shadow-xs focus:ring-purple-500 focus:border-purple-500" />
                        </div>
                        <div>
                            <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-2">Heure de Fin *</label>
                            <input type="time" name="end_time" required value="{{ old('end_time', substr($schedule->end_time, 0, 5)) }}"
                                   class="w-full text-sm font-bold border-slate-300 rounded-xl shadow-xs focus:ring-purple-500 focus:border-purple-500" />
                        </div>
                    </div>
