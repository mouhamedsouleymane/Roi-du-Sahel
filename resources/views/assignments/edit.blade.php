            ...
            <div class="bg-white shadow-xl rounded-3xl p-8 border border-slate-200/80">
                <form method="POST" action="{{ route('assignments.update', $assignment) }}" class="space-y-6">
                        ...
                        <select name="teacher_id" required class="w-full text-sm font-bold border-slate-300 rounded-xl shadow-xs focus:ring-purple-500 focus:border-purple-500">
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->id }}" {{ old('teacher_id', $assignment->teacher_id) == $teacher->id ? 'selected' : '' }}>
                                    {{ $teacher->user->name }} ({{ $teacher->speciality }})
                                </option>
                            @endforeach
                        </select>
                        @error('teacher_id') <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span> @enderror
                    </div>
                        ...
                        <div>
                            ...
                            <select name="class_id" required class="w-full text-sm font-bold border-slate-300 rounded-xl shadow-xs focus:ring-purple-500 focus:border-purple-500">
                                @foreach ($classes as $cls)
                                    <option value="{{ $cls->id }}" {{ old('class_id', $assignment->class_id) == $cls->id ? 'selected' : '' }}>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                        <a href="{{ route('assignments.index') }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition">
                            Annuler
                        </a>
