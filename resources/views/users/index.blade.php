<x-app-layout>
    ...
    <div class="py-8 bg-slate-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <!-- Alerts -->
            @if (session('status'))
                <div class="bg-emerald-100 border border-emerald-400 text-emerald-800 px-4 py-3 rounded-2xl font-bold text-sm shadow-sm" role="alert">
                    ✅ {{ session('status') }}
                </div>
            @endif

            @if ($errors->any())
                <div class="bg-red-100 border border-red-400 text-red-800 px-4 py-3 rounded-2xl font-bold text-sm shadow-sm" role="alert">
                    ⚠️ {{ $errors->first() }}
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="p-6 bg-white rounded-3xl shadow-sm border border-slate-200/80 flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-purple-100 text-purple-900 font-black flex items-center justify-center text-2xl">
                        👤
                    </div>
                    <div>
                        <div class="text-xs font-extrabold text-slate-400 uppercase tracking-wider">Comptes Utilisateurs</div>
                        <div class="text-2xl font-black text-purple-950">{{ $users->total() }}</div>
                    </div>
                </div>

                <div class="p-6 bg-white rounded-3xl shadow-sm border border-slate-200/80 flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-amber-100 text-amber-900 font-black flex items-center justify-center text-2xl">
                        🛡️
                    </div>
                    <div>
                        <div class="text-xs font-extrabold text-slate-400 uppercase tracking-wider">Rôles Configuration</div>
                        <div class="text-2xl font-black text-amber-900">{{ $totalRolesCount }} Rôles Défini(s)</div>
                    </div>
                </div>

                <div class="p-6 bg-white rounded-3xl shadow-sm border border-slate-200/80 flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-emerald-100 text-emerald-900 font-black flex items-center justify-center text-2xl">
                        🔐
                    </div>
                    <div>
                        <div class="text-xs font-extrabold text-slate-400 uppercase tracking-wider">Permissions Système</div>
                        <div class="text-2xl font-black text-emerald-900">{{ $totalPermissionsCount }} Habilitations</div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 bg-white shadow-sm rounded-3xl border border-slate-200/80">
                <form method="GET" action="{{ route('users.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    <div>
                        <label for="search" class="block font-black text-xs text-slate-600 uppercase tracking-wider mb-1">
                            Recherche (Nom, Email)
                        </label>
                    ...
                    <div>
                        ...
                        <select id="role" name="role" class="w-full text-sm font-bold rounded-xl border-slate-300 shadow-xs focus:border-purple-500 focus:ring-purple-500">
                    ...
                    <div class="flex items-center gap-2">
                        <button type="submit" class="px-5 py-2.5 bg-purple-950 hover:bg-purple-900 text-white font-black rounded-xl text-xs transition shadow-md">
                            🔍 Filtrer
                        </button>
                        <a href="{{ route('users.index') }}" class="px-5 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold rounded-xl text-xs transition">
                            Réinitialiser
                        </a>
                    </div>
                </form>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-6 py-4 text-left text-xs font-black text-slate-500 uppercase tracking-wider">Utilisateur</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-slate-500 uppercase tracking-wider">Email</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-slate-500 uppercase tracking-wider">Rôle(s) Attribué(s)</th>
                        ...
                        <tbody class="bg-white divide-y divide-slate-200">
                            @forelse ($users as $u)
                                <tr class="hover:bg-purple-50/30 transition">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-900 flex items-center gap-3">
                                        <div class="w-9 h-9 rounded-full bg-purple-900 text-amber-400 font-black flex items-center justify-center text-xs shadow-md ring-2 ring-purple-200">
                                            {{ mb_substr($u->name, 0, 1) }}
                                        </div>
                                        <div>
                                            <div class="font-extrabold text-purple-950">{{ $u->name }}</div>
                                            @if ($u->id === Auth::id())
                                                <span class="text-[10px] font-black px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800">Votre compte</span>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <div class="flex flex-wrap gap-1">
                                            @forelse ($u->roles as $userRole)
                                                <span class="px-3 py-1 text-xs font-black rounded-full shadow-xs
